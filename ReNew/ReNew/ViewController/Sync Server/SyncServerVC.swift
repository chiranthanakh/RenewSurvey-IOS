...
extension SyncServerVC {
    ...
    func callSyncAPI() {
        ...
        APIManager.sharedInstance.requestWithPostMultipartParam(endpointurl: AppConstant.API.kSyncDataFromServer, parameters: param as NSDictionary, isShowLoader: true) { error, dict in
            if let error = error {
    ...
    func createTable(tableData: ModelServerSyncTable) {
        ...
        for i in 0..<tableData.columns.count{
            ...
            if tableData.columns[i].type == "int" || tableData.columns[i].type.contains("tinyint") || tableData.columns[i].type.contains("int"){
